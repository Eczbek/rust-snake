...
fn move_wrap(position: &mut Position, window_size: &terminal::WindowSize, (x, y): (i16, i16)) {
    position.x =
        ((position.x as i16 + x + window_size.columns as i16) % window_size.columns as i16) as u16;
    position.y =
        ((position.y as i16 + y + window_size.rows as i16) % window_size.rows as i16) as u16;
}
...
fn main() -> io::Result<()> {
    ...
    'game: loop {
        ...
        queue!(
            stdout,
            cursor::MoveTo(head.x * 2, head.y),
            style::SetBackgroundColor(style::Color::Blue),
            style::Print("  "),
            style::ResetColor,
            cursor::MoveTo(apple.x * 2, apple.y),
